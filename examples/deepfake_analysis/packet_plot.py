...
def load_image(path_to_file: str) -> jnp.ndarray:
    
    image = Image.open(path_to_file)
    tensor = np.nan_to_num(np.array(image), posinf=255, neginf=0)
    return tensor

def process_images(tensor: jnp.ndarray, paths: list) -> jnp.ndarray:
    tensor = jnp.mean(tensor/255., -1)
    packets = jaxwt.packets.WaveletPacket2D(tensor, pywt.Wavelet("Haar"),
        max_level=3)
...
if __name__ == '__main__':
    ...
    ffhq_images = np.array_split(np.stack(ffhq_images), 20)
    ffhq_packets = []
    for image_batch in tqdm(ffhq_images):
        ffhq_packets.append(process_images(jnp.array(image_batch),
            natural_path).astype(jnp.float16))

    ffhq_packets = jnp.concatenate(ffhq_packets).astype(jnp.float32)
    mean_packets_ffhq = jnp.mean(ffhq_packets, 0)
    del ffhq_images
    del ffhq_packets
    ...
    gan_images = np.array_split(np.stack(gan_images), 20)
    gan_packets = []
    for image_batch in tqdm(gan_images):
        gan_packets.append(process_images(jnp.array(image_batch),
            natural_path).astype(jnp.float16))

    mean_packets_gan = jnp.mean(jnp.concatenate(gan_packets).astype(jnp.float32), 0)
    del gan_images
    del gan_packets
    ...
    plt.plot(jnp.mean(mean_packets_ffhq, (1, 2)).flatten(), label='real')
    plt.plot(jnp.mean(mean_packets_gan, (1, 2)).flatten(), label='fake')
